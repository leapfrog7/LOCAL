import { privateDocumentPresentation } from '../../services/privateDocumentPresentation'
...
import { pagePreviewSource } from '../viewer/pagePreviewSource'
...
export function ActionsScreen(props: ActionsScreenProps) {
  ...
  return <>{mode ? <ActionWorkspace mode={mode} {...props} onBack={() => setMode(null)} /> : <>
    <header className="page-header"><div className="brand-mark"><SlidersHorizontal /></div><div><h1>Document tools</h1><p>Make changes to documents already in LOCAL</p></div></header>
    <section className="actions-intro"><strong>Choose what you want to do</strong><span>Combine, reorder, clean or export documents without uploading them. Originals remain unchanged unless you explicitly replace one.</span></section>
...
function ActionWorkspace({ mode, documents, onBack, onCombine, onExtract, onReorder, onCompress, onInsert, onClean, onAnalyse, onRename, onCopyText, onExportText }: ActionsScreenProps & { mode: ActionMode; onBack: () => void }) {
  ...
  const presented = useMemo(() => new Map(documents.map(item => [item.id, privateDocumentPresentation(item)])), [documents])
  const document = chosen[0]
  const displayDocument = document ? presented.get(document.id)! : undefined
  const analyseSelected = useCallback(() => {
    if (!document) return Promise.reject(new Error('Choose a document first.'))
    return onAnalyse(document)
  }, [document, onAnalyse])
  ...
  const run = async (action: () => Promise<void>) => { setBusy(true); setError(''); setMessage(''); try { await action(); setMessage(mode === 'compress' ? 'Compressed PDF saved to Downloads.' : mode === 'export' ? 'Recognised text exported successfully.' : mode === 'rename' ? `${chosen.length} document${chosen.length === 1 ? '' : 's'} renamed.` : 'Action completed successfully.') } catch (cause) { setError(cause instanceof Error ? cause.message : 'This action could not be completed.') } finally { setBusy(false) } }
  if (document && mode === 'split') return <OperationShell title={title} onBack={() => setSelected([])}><PageExtractSheet document={displayDocument!} initialPage={0} busy={busy} error={error} onClose={() => setSelected([])} onExtract={indexes => void run(() => onExtract(document, indexes))} /></OperationShell>
  if (document && mode === 'reorder') return <OperationShell title={title} onBack={() => setSelected([])}><ReorderPagesPanel document={displayDocument!} busy={busy} error={error} onSave={(indexes, saveMode) => void run(() => onReorder(document, indexes, saveMode))} /></OperationShell>
...
function DocumentPicker({ mode, documents: sourceDocuments, selected, multi, onToggle }: { mode: ActionMode; documents: VaultDocument[]; selected: string[]; multi: boolean; onToggle: (id: string) => void }) {
  const documents = useMemo(() => sourceDocuments.map(privateDocumentPresentation), [sourceDocuments])
  ...
  const filtered = useMemo(() => {
    ...
    const matches = documents.filter(document => {
      ...
      if (document.isPrivate && (deferredQuery || tag !== 'All tags' || type !== 'All types')) return false
