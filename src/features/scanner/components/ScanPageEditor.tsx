...
export function ScanPageEditor({ page, pageNumber, initialMode = 'crop', onCancel, onSave }: { page: DocumentPage; pageNumber: number; initialMode?: 'crop' | 'preview'; onCancel: () => void; onSave: (page: DocumentPage) => void }) {
  const original = page.originalImageUrl ?? page.imageUrl
  const [corners, setCorners] = useState<PageCorners>(page.corners ?? defaultCorners), [preset, setPreset] = useState<RenderPreset>(page.renderPreset ?? 'auto')
  ...
  const [mode, setMode] = useState<'crop' | 'preview'>(initialMode), [processing, setProcessing] = useState(false), [adjustOpen, setAdjustOpen] = useState(false), [error, setError] = useState('')

  const render = async (nextPreset = preset, nextAdjustments = adjustments) => {
    setProcessing(true); setError('')
    try { const result = await processPageImages(original, corners, nextPreset, nextAdjustments); setPreview(result.imageUrl); setPreset(nextPreset); setMode('preview') }
  ...
  const save = async () => {
    setProcessing(true); setError('')
    try {
      const result = await processPageImages(original, corners, preset, adjustments)
      onSave({ ...page, originalImageUrl: original, ...result, imagePath: undefined, thumbnailPath: undefined, ocrImagePath: undefined, corners, renderPreset: preset, adjustments, processingState: 'processed', ocrState: 'pending', ocrText: '' })
    } catch (renderError) {
      console.error('LOCAL page save processing failed', renderError)
      setError('This crop could not be applied. Your previous page is still safe.')
      setProcessing(false)
    }
  }

  return <div className="scan-editor-screen">
    <header className="scan-editor-header"><button onClick={onCancel} aria-label="Back to page review"><ChevronLeft /></button><div><strong>Page {pageNumber}</strong><span>{mode === 'crop' ? 'Confirm detected edges' : 'Choose the best finish'}</span></div><button className="editor-done" onClick={() => void save()} disabled={processing}><Check /> Done</button></header>
