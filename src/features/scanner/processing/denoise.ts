...
export function reduceNoise(canvas: HTMLCanvasElement) {
  const context = canvas.getContext('2d'); if (!context) return
  const source = cloneCanvas(canvas)
  context.save(); context.globalAlpha = 0.2; context.filter = 'blur(0.45px)'; context.drawImage(source, 0, 0); context.restore()
}

export function sharpenConservatively(canvas: HTMLCanvasElement) {
  const context = canvas.getContext('2d'); if (!context) return
  ...
  for (let index = 0; index < original.data.length; index += 4) for (let channel = 0; channel < 3; channel += 1) {
    ...
    original.data[index + channel] = Math.max(0, Math.min(255, original.data[index + channel] + detail * 0.28))
  }
