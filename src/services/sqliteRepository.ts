...
async function getConnection() {
  ...
  if (!connectionPromise)
    connectionPromise = (async () => {
      ...
      try {
        ...
        if (needsMigration) {
          try {
            ...
            /* The failed migration may not have opened a connection. */
          }
      ...
      if (!columns.has('deleted_at')) await db.execute('ALTER TABLE documents ADD COLUMN deleted_at TEXT;')
...
function pageFromRow(row: Row): DocumentPage {
  return {
    ...
    originalImagePath: optionalText(row.original_image_path),
    thumbnailPath: optionalText(row.thumbnail_path),
    ocrImagePath: optionalText(row.ocr_image_path),
    rotation: number(row.rotation),
    ocrText: text(row.ocr_text),
    ocrState: text(row.ocr_state) as DocumentPage['ocrState'],
...
async function documentsFromRows(rows: Row[]) {
  ...
  for (const row of rows) {
    ...
    documents.push({
      id: text(row.id),
      storageVersion: number(row.storage_version),
      title: text(row.title),
      titleSource: optionalText(row.title_source) as VaultDocument['titleSource'],
      smartMetadata: parseJson<DocumentSmartMetadata | undefined>(row.smart_metadata_json, undefined),
      ...
      storageProtection: optionalText(row.storage_protection) as VaultDocument['storageProtection'],
      pdfPasswordProtected: number(row.pdf_password_protected) === 1,
      pdfPath: optionalText(row.pdf_path),
      pdfGeneratedAt: optionalText(row.pdf_generated_at),
      processingStage: optionalText(row.processing_stage) as VaultDocument['processingStage'],
...
export const sqliteRepository = {
  ...
  async securityStatus() {
    ...
    await getConnection()
    return {
      databaseEncrypted: Boolean((await sqlite.isDatabaseEncrypted(DB_NAME)).result)
    }
  },

  async list() {
    const db = await getConnection(),
      rows = (await db.query('SELECT * FROM documents ORDER BY updated_at DESC')).values ?? []
    return documentsFromRows(rows)
  },

  async get(id: string) {
    const db = await getConnection(),
      rows = (await db.query('SELECT * FROM documents WHERE id = ?', [id])).values ?? []
  ...
  async remove(id: string) {
    ...
    if (ftsAvailable) await db.run('DELETE FROM document_search WHERE document_id = ?', [id])
  },
  ...
  async getSetting(key: string) {
    const db = await getConnection(),
      row = ((await db.query('SELECT value FROM settings WHERE key = ?', [key])).values ?? [])[0]
  ...
  async listFolders() {
    const db = await getConnection()
    return ((await db.query('SELECT name FROM folders ORDER BY lower(name)')).values ?? []).map((row) => text(row.name)).filter(Boolean)
  },
  async createFolder(name: string) {
    const db = await getConnection()
    await db.run('INSERT OR IGNORE INTO folders(name,created_at) VALUES(?,?)', [name, new Date().toISOString()])
  },
  async removeFolder(name: string) {
    const db = await getConnection()
    await db.run('DELETE FROM folders WHERE name = ?', [name])
  },
  ...
  async upsertJob(job: ProcessingJob) {
    return serializeWrite(async () => {
      const db = await getConnection()
      await db.run(
        `INSERT INTO processing_jobs(id,document_id,page_id,type,status,attempts,progress,error,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)
  ...
  async pendingJobs() {
    return serializeWrite(async () => {
      const db = await getConnection()
      await db.run("UPDATE processing_jobs SET status='pending' WHERE status='processing'", [], false)
      ...
      return rows.map((row) => ({
        ...
        pageId: optionalText(row.page_id),
        type: text(row.type) as ProcessingJobType,
        status: text(row.status) as ProcessingJobStatus,
  ...
  async cancelJobs(documentId: string) {
    return serializeWrite(async () => {
      const db = await getConnection()
      await db.run("UPDATE processing_jobs SET status='cancelled', updated_at=? WHERE document_id=? AND status IN ('pending','processing')", [new Date().toISOString(), documentId], false)
    })
  },
  async jobsCancelled(documentId: string) {
    const db = await getConnection()
    const rows = (await db.query('SELECT status FROM processing_jobs WHERE document_id=?', [documentId])).values ?? []
    return rows.some((row) => text(row.status) === 'cancelled') && rows.every((row) => ['cancelled', 'complete'].includes(text(row.status)))
  }
}
